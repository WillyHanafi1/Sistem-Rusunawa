        ...
        <div className="p-8">
            <div className="flex items-center justify-between mb-6">
                <div>
                    ...
                    <p className="text-slate-400 text-sm mt-1">{invoices.length} tagihan</p>
                </div>
            ...
            {loading ? (
                ...
                <div className="bg-white/5 border border-white/10 rounded-2xl overflow-hidden">
                        ...
                        <tbody>
                            {invoices.map((inv) => (
                                <tr key={inv.id} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                                    <td className="px-6 py-4 text-slate-400 text-xs">#{inv.id}</td>
                                    ...
                                    <td className="px-6 py-4">
                                        {inv.payment_url && (
                                            <a href={inv.payment_url} target="_blank" className="text-blue-400 hover:text-blue-300 text-xs underline">Link Bayar</a>
                                        )}
                                    </td>
                                </tr>
                            ))}
                    ...
                    {invoices.length === 0 && <p className="text-center text-slate-500 py-12">Belum ada tagihan</p>}
                </div>
            )}
                ...
                <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                    <div className="bg-slate-900 border border-white/10 rounded-2xl w-full max-w-md p-6">
                        <div className="flex items-center justify-between mb-5">
                            <h2 className="text-white font-semibold">Generate Tagihan</h2>
                            <button onClick={() => setModal(false)} className="text-slate-400 hover:text-white"><X className="w-5 h-5" /></button>
                        </div>
                        <div className="space-y-4">
                            <div>
                                <label className="block text-slate-300 text-sm mb-1.5">Penghuni</label>
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                <div>
                                    <label className="block text-slate-300 text-sm mb-1.5">Bulan</label>
                                ...
                                <div>
                                    <label className="block text-slate-300 text-sm mb-1.5">Tahun</label>
                            ...
                            <div>
                                <label className="block text-slate-300 text-sm mb-1.5">Tanggal Jatuh Tempo</label>
