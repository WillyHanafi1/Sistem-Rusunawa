    ...
    return (
        <div className="p-8">
            <div className="flex items-center justify-between mb-6">
                <div>
                    <h1 className="text-2xl font-bold text-white">Manajemen Penghuni</h1>
                    <p className="text-slate-400 text-sm mt-1">{tenants.filter(t => t.is_active).length} penghuni aktif</p>
                </div>
            ...
            {loading ? (
                ...
                <div className="bg-white/5 border border-white/10 rounded-2xl overflow-hidden">
                        ...
                        <tbody>
                            {tenants.map((t) => (
                                <tr key={t.id} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                                    <td className="px-6 py-4 text-slate-400 text-xs">#{t.id}</td>
                                    <td className="px-6 py-4 text-white">User #{t.user_id}</td>
                                    <td className="px-6 py-4 text-slate-300">Kamar #{t.room_id}</td>
                                    <td className="px-6 py-4 text-slate-300">{t.contract_start}</td>
                                    <td className="px-6 py-4 text-slate-300">{t.contract_end}</td>
                                    ...
                                    <td className="px-6 py-4">
                                        {t.is_active && (
                                            <button onClick={() => handleDeactivate(t.id)}
                                                className="text-xs text-red-400 hover:text-red-300 hover:bg-red-500/10 px-2.5 py-1 rounded-lg transition-all">
                                                Nonaktifkan
                                            </button>
                                        )}
                                    </td>
                    ...
                    {tenants.length === 0 && <p className="text-center text-slate-500 py-12">Belum ada data penghuni</p>}
                </div>
            )}
                ...
                <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                    <div className="bg-slate-900 border border-white/10 rounded-2xl w-full max-w-md p-6">
                        <div className="flex items-center justify-between mb-5">
                            <h2 className="text-white font-semibold">Tambah Penghuni</h2>
                            <button onClick={() => setModal(false)} className="text-slate-400 hover:text-white"><X className="w-5 h-5" /></button>
                        </div>
                        <p className="text-slate-400 text-xs mb-4">Pastikan user penghuni sudah dibuat di sistem (via Register Admin) sebelum assign ke kamar.</p>
                            ...
                            {[
                                ...
                                <div key={field}>
                                    ...
                                    <input type={type} value={(form as any)[field]}
                                        ...
                                        className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-white focus:outline-none focus:border-blue-500 transition-all"
                                    />
                                </div>
                            ))}
                        ...
                        <div className="flex gap-3 mt-6">
                            <button onClick={() => setModal(false)} className="flex-1 py-2.5 border border-white/10 text-slate-300 rounded-xl text-sm transition-all">Batal</button>
